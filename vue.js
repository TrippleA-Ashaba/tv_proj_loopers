var Vue = (function (exports) {
	...
	function setDevtoolsHook(hook, target) {
		...
		if (exports.devtools) {
			...
			window.HTMLElement &&
			// also exclude jsdom
			!((_b = (_a = window.navigator) === null || _a === void 0 ? void 0 : _a.userAgent) ===
				null || _b === void 0
				? void 0
				: _b.includes("jsdom"))
		) {
	...
	const isElementRoot = (vnode) => {
		return (
			vnode.shapeFlag & (6 /* ShapeFlags.COMPONENT */ | 1) /* ShapeFlags.ELEMENT */ ||
	...
	function doWatch(source, cb, { immediate, deep, flush, onTrack, onTrigger } = EMPTY_OBJ) {
		...
		if (isRef(source)) {
			...
			if (cb) {
				// getter with cb
				getter = () =>
					callWithErrorHandling(source, instance, 2 /* ErrorCodes.WATCH_GETTER */);
			} else {
				// no cb -> simple effect
				getter = () => {
  ...
  */
			...
			// #5405 in dev, always hydrate children for HMR
			{
				...
				if (props) {
					if (
						...
						!optimized ||
						patchFlag &
							(16 /* PatchFlags.FULL_PROPS */ | 32) /* PatchFlags.HYDRATE_EVENTS */
					) {
	...
	function baseCreateRenderer(options, createHydrationFns) {
		...
		const unmount = (
			...
			const { type, props, ref, children, dynamicChildren, shapeFlag, patchFlag, dirs } =
				vnode;
			// unset ref
			if (ref != null) {
	...
	function isText(node) {
		return (
			node.type === 5 /* NodeTypes.INTERPOLATION */ || node.type === 2 /* NodeTypes.TEXT */
		);
	}
	function isVSlot(p) {
	...
	function isSlotOutlet(node) {
		return (
			node.type === 1 /* NodeTypes.ELEMENT */ && node.tagType === 2 /* ElementTypes.SLOT */
		);
	}
	...
	function parseText(context, mode) {
		const endTokens =
			mode === 3 /* TextModes.CDATA */ ? ["]]>"] : ["<", context.options.delimiters[0]];
		...
		if (dir.name === "if") {
			...
			while (i-- >= -1) {
				...
				if (sibling && sibling.type === 9 /* NodeTypes.IF */) {
					...
					context.onError(
						createCompilerError(30 /* ErrorCodes.X_V_ELSE_NO_ADJACENT_IF */, node.loc)
					);
				}
				break;
			}
	...
	function processFor(node, dir, context, processCodegen) {
		...
		if (!parseResult) {
			context.onError(
				createCompilerError(32 /* ErrorCodes.X_V_FOR_MALFORMED_EXPRESSION */, dir.loc)
			);
			return;
		}
	...
	function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
		...
		for (let i = 0; i < children.length; i++) {
			...
			if (onComponentSlot) {
				...
				context.onError(
					createCompilerError(37 /* ErrorCodes.X_V_SLOT_MIXED_SLOT_USAGE */, slotDir.loc)
				);
				break;
			}
			...
			if ((vIf = findDir(slotElement, "if"))) {
				...
				if (prev && isTemplateNode(prev) && findDir(prev, "if")) {
					...
					while (
						conditional.alternate.type === 19 /* NodeTypes.JS_CONDITIONAL_EXPRESSION */
					) {
					...
					context.onError(
						createCompilerError(30 /* ErrorCodes.X_V_ELSE_NO_ADJACENT_IF */, vElse.loc)
					);
				}
	...
	const transformModel = (dir, node, context) => {
		...
		if (!expString.trim() || (!isMemberExpression(expString) && !maybeRef)) {
			context.onError(
				createCompilerError(42 /* ErrorCodes.X_V_MODEL_MALFORMED_EXPRESSION */, exp.loc)
			);
			return createTransformProps();
		}
	...
	const transformModel$1 = (dir, node, context) => {
		...
		if (dir.arg) {
			context.onError(
				createDOMCompilerError(55 /* DOMErrorCodes.X_V_MODEL_ARG_ON_ELEMENT */, dir.arg.loc)
			);
		}
		...
		if (tag === "input" || tag === "textarea" || tag === "select" || isCustomElement) {
			...
			context.onError(
				createDOMCompilerError(54 /* DOMErrorCodes.X_V_MODEL_ON_INVALID_ELEMENT */, dir.loc)
			);
		}
	...
	const ignoreSideEffectTags = (node, context) => {
		if (
			...
			context.onError(
				createDOMCompilerError(60 /* DOMErrorCodes.X_IGNORED_SIDE_EFFECT_TAG */, node.loc)
			);
			context.removeNode();
		}
	};
